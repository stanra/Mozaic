...
def cut(picture, param_x, param_y, mode, fixed_size):
    if mode == 'number':
        ...
        num_tile = tuple(int(math.ceil(p_s/t_s)) for p_s, t_s in zip(picture.shape, tile_size))
        fraction = tuple((0,num) for num in num_tile) # maybe should num-1 but it doesn't actually change anything. actually better like that. The last picture 'wants' to be same size.
    else:
...
def stitch(mozaic, info, top_left, down_right, fixed_size):
    ...
    print(top_left_tile)
    print(down_right_tile)
    lines = []
    i = 0
    for tile_x in range(top_left_tile[0], down_right_tile[0]+1):
        cur_line = mozaic[(tile_x,top_left_tile[1])]
        for tile_y in range(top_left_tile[1], down_right_tile[1]+1):
...
def _get_tile_including(point, info):
    ...
    tile_size_avg = tuple(pic_size / num_tile for pic_size, num_tile in zip(info['original_size'], info['num_tiles']))
    ...
    for i in (0,1): # We treat x and y separately
        ...
        while True:
            ...
            pos_guessed_tile = num_finished_cycle * q * tile_size_avg[i] +\
...
if __name__ == '__main__':
    pic = np.reshape(['({},{})'.format(i, j) for i in range(79) for j in range(100)], (79, 100))
    moz, info = cut(pic, 60, 7, 'number', False)
    res = stitch(moz, info, (0, 0), (65, 19), False)
    print(res)